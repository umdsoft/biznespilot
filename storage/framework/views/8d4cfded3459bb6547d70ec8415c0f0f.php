...
function openFeatureModal(featureKey) {
    ...
    if (modal) {
        ...
        const closeBtn = modal.querySelector('button[onclick*="closeFeatureModal"]');
...
function closeFeatureModal(featureKey) {
    ...
    if (modal) {
        const content = modal.querySelector('.relative.w-full');
        if (content) content.classList.remove('animate-modal-in');
...
// Focus trap: keep Tab navigation within open modal
document.addEventListener('keydown', function(e) {
    if (e.key !== 'Tab') return;
    var openModal = document.querySelector('[id^="modal-"]:not(.hidden)');
    if (!openModal) return;
    var focusable = openModal.querySelectorAll('button, a, input, [tabindex]:not([tabindex="-1"])');
