...
const duplicateOffer = () => {
    router.post(route('marketing.offers.duplicate', props.offer.id));
};

const deleteOffer = () => {
    if (confirm(`${props.offer.name} nomli taklifni o'chirishni xohlaysizmi?`)) {
        router.delete(route('marketing.offers.destroy', props.offer.id));
    }
};
...
const generateVariations = async () => {
    ...
    try {
        const response = await fetch(route('marketing.offers.generate-variations', props.offer.id), {
            method: 'POST',
            headers: {
...
<template>
    <MarketingLayout :title="offer.name">
                    ...
                    <div class="bg-gradient-to-r from-purple-600 to-indigo-700 rounded-xl shadow-xl p-8 text-white">
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex gap-2">
                                <Link
                                    :href="route('marketing.offers.edit', offer.id)"
