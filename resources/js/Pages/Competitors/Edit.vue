<template>
  <BusinessLayout title="Raqibni Tahrirlash">
    <div class="max-w-4xl mx-auto">
      <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-900">Raqibni Tahrirlash</h2>
        <p class="mt-1 text-sm text-gray-600">{{ competitor.name }} ma'lumotlarini yangilash</p>
      </div>
...
const form = useForm({
  ...
  products: props.competitor.products || [],
  pricing: props.competitor.pricing || [],
  marketing_strategies: props.competitor.marketing_strategies || [],
  threat_level: props.competitor.threat_level,
  is_active: props.competitor.is_active,
});

const submit = () => {
  form.put(`/competitors/${props.competitor.id}`);
};

const confirmDelete = () => {
  if (confirm(`Rostdan ham "${props.competitor.name}" raqibini o'chirmoqchimisiz?`)) {
    router.delete(`/competitors/${props.competitor.id}`);
  }
};
</script>
