...
// Use shared integrations route for sync
const syncData = () => {
...
// Connect to YouTube via shared integrations route
const connectYouTube = async () => {
    isConnecting.value = true;
    try {
        const response = await fetch('/integrations/youtube/auth-url', {
            method: 'GET',
            headers: {
...
const disconnectYouTube = () => {
    if (!confirm('YouTube integratsiyasini uzmoqchimisiz?')) return;
...
<template>
    ...
    <component :is="layoutComponent" title="YouTube Analitika">
        <div class="space-y-6">
            <!-- Flash Messages -->
                ...
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <button
                            ...
                            class="inline-flex items-center mt-3 text-sm text-yellow-400 hover:text-yellow-300 disabled:opacity-70"
                        >
                            {{ isConnecting ? 'Ulanmoqda...' : 'Qaytadan ulash uchun bu yerga bosing' }}
                            <svg v-if="!isConnecting" class="w-4 h-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div v-if="!integration || !integration.is_connected" class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-12 text-center">
                ...
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-3">YouTube Ulang</h2>
                <p class="text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto">
                    YouTube kanalingizni ulang va batafsil tahlillarni ko'ring.
                </p>
                <button
                    @click="connectYouTube"
                    :disabled="isConnecting"
                    class="inline-flex items-center px-6 py-3 bg-red-500 text-white font-semibold rounded-xl hover:bg-red-600 transition-colors disabled:opacity-70 disabled:cursor-wait"
                >
                    ...
                    {{ isConnecting ? 'Ulanmoqda...' : 'YouTube Ulang' }}
                </button>
            </div>
            ...
            <template v-else>
                            ...
                            <div>
                                ...
                                <div class="space-y-3">
                                    <Link
                                        ...
                                        :key="video.id"
                                        :href="route('integrations.youtube.video', { videoId: video.id })"
                ...
                <div class="text-center">
                    ...
                    >
                        ...
                        YouTube integratsiyasini uzish
                    </button>
                </div>
            </template>
        </div>
