...
const getEngagementBg = (rate) => {
    if (rate >= 5) return 'bg-green-100 border-green-200';
    if (rate >= 3) return 'bg-blue-100 border-blue-200';
    if (rate >= 1) return 'bg-yellow-100 border-yellow-200';
...
<template>
    ...
    <BusinessLayout>
        <div class="min-h-screen bg-gradient-to-br from-gray-50 via-pink-50/30 to-purple-50/30">
            <!-- Header -->
            <div class="bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-600 relative overflow-hidden">
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h1 class="text-2xl md:text-3xl font-bold text-white">Instagram Tahlili</h1>
                                <p class="text-pink-100 text-sm mt-1">Akkountingiz qanday ishlayotganini ko'ring</p>
                            </div>
                        </div>
                        <div class="flex flex-wrap items-center gap-3">
                            <select v-if="hasAccount" v-model="dateRange"
                                class="px-4 py-2.5 bg-white/20 backdrop-blur-sm border-0 rounded-xl text-white text-sm focus:ring-2 focus:ring-white/50 cursor-pointer">
                                <option v-for="opt in dateRangeOptions" :key="opt.value" :value="opt.value" class="text-gray-900">
                                    {{ opt.label }}
                                </option>
                            </select>
                            <button v-if="hasAccount" @click="syncData" :disabled="syncing"
                                class="px-4 py-2.5 bg-white/20 backdrop-blur-sm text-white rounded-xl hover:bg-white/30 disabled:opacity-50 flex items-center gap-2 text-sm transition-all">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <!-- Not Connected State -->
                <div v-if="!isConnected" class="bg-white rounded-3xl shadow-xl p-8 md:p-12 text-center">
                    <div class="mx-auto w-24 h-24 bg-gradient-to-br from-pink-500 to-purple-600 rounded-full flex items-center justify-center mb-6 shadow-2xl">
                    ...
                    <a href="/business/target-analysis"
                        class="inline-flex items-center px-8 py-4 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-2xl hover:from-pink-600 hover:to-purple-700 font-bold shadow-xl transition-all hover:scale-105">
                ...
                <div v-else-if="isConnected && !hasAccount" class="bg-white rounded-3xl shadow-xl p-8 md:p-12 text-center">
                    <div class="mx-auto w-24 h-24 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-full flex items-center justify-center mb-6 shadow-2xl">
                        <svg class="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                        </svg>
                    </div>
                    <h3 class="text-2xl font-bold text-gray-900 mb-4">Instagram ma'lumotlari topilmadi</h3>
                    <p class="text-gray-600 mb-6 max-w-lg mx-auto text-lg">
                    ...
                    <div v-if="error" class="mb-6 max-w-xl mx-auto">
                        <div class="p-5 bg-red-50 border border-red-200 rounded-2xl text-left">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="font-bold text-red-800 text-lg">Xatolik yuz berdi</p>
                                    <p class="text-red-700 mt-2">{{ error }}</p>

                                    <!-- Permission issue - needs reconnect -->
                                    <div v-if="syncError?.needs_reconnect" class="mt-4 p-4 bg-white rounded-xl border border-red-100">
                                        <p class="font-semibold text-gray-900 mb-2">Yechim:</p>
                                        <p class="text-sm text-gray-600 mb-3">
                                            Meta integratsiyasini qayta ulashingiz kerak. Qayta ulash paytida Instagram ruxsatlari avtomatik so'raladi.
                                        </p>
                                        <div class="flex flex-wrap gap-3">
                                            <button @click="reconnectMeta" :disabled="reconnecting"
                                                class="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl text-sm font-semibold hover:from-blue-600 hover:to-blue-700 disabled:opacity-50 transition-all">
                                    ...
                                    <!-- No pages -->
                                    <div v-else-if="syncError?.no_pages" class="mt-4 p-4 bg-white rounded-xl border border-red-100">
                                        <p class="font-semibold text-gray-900 mb-2">Yechim:</p>
                                        <p class="text-sm text-gray-600">Facebook Business sahifasi yarating va Instagram akkountingizni unga ulang.</p>
                                        <a href="https://business.facebook.com" target="_blank" class="mt-3 inline-flex items-center gap-2 text-blue-600 text-sm font-medium hover:underline">
                                    ...
                                    <div v-else-if="syncError?.no_instagram" class="mt-4 p-4 bg-white rounded-xl border border-red-100">
                                        <p class="font-semibold text-gray-900 mb-2">Yechim:</p>
                                        <p class="text-sm text-gray-600 mb-2">Facebook sahifangizga Instagram Business akkountini ulang:</p>
                                        ...
                                        <p v-if="syncError?.pages" class="mt-3 text-xs text-gray-500">
                    ...
                    <button @click="syncData" :disabled="syncing"
                        class="inline-flex items-center px-8 py-4 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-2xl hover:from-pink-600 hover:to-purple-700 font-bold shadow-xl transition-all hover:scale-105 disabled:opacity-50">
                        <svg :class="{ 'animate-spin': syncing }" class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                        </svg>
                        {{ syncing ? 'Sinxronizatsiya qilinmoqda...' : 'Instagram sinxronizatsiya qilish' }}
                    </button>

                    <p v-if="syncing" class="mt-4 text-sm text-gray-500">
                        Bu bir necha daqiqa davom etishi mumkin...
                    </p>
                </div>

                <!-- Account Selector -->
                <div v-else-if="instagramAccounts?.length > 1" class="bg-white rounded-2xl shadow-lg p-5 mb-6">
                    <label class="block text-sm font-semibold text-gray-700 mb-3">Akkount tanlang:</label>
                    <div class="flex flex-wrap gap-3">
                        <button v-for="account in instagramAccounts" :key="account.id"
                            ...
                            <img v-if="account.profile_picture_url" :src="account.profile_picture_url"
                                :alt="account.username" class="w-12 h-12 rounded-full ring-2 ring-white shadow" />
                            <div class="text-left">
                                <div class="font-bold text-gray-900">@{{ account.username }}</div>
                                <div class="text-sm text-gray-500">{{ formatNumber(account.followers_count) }} follower</div>
                            </div>
                            <svg v-if="account.is_primary" class="w-6 h-6 text-pink-500 ml-2" fill="currentColor" viewBox="0 0 20 20">
                ...
                <!-- Loading State -->
                <div v-if="loading && !dataLoaded" class="bg-white rounded-3xl shadow-xl p-12 text-center">
                    ...
                    <p class="text-gray-500 text-lg">Ma'lumotlar yuklanmoqda...</p>
                </div>
                ...
                <div v-else-if="hasAccount && dataLoaded" class="space-y-6">
                    ...
                    <div class="bg-white rounded-3xl shadow-xl overflow-hidden">
                        <div class="bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 p-1">
                                    ...
                                    <div class="flex flex-col lg:flex-row lg:items-center gap-6">
                                        ...
                                        <div class="flex items-center gap-5">
                                            <div class="relative">
                                                <img v-if="selectedAccount?.profile_picture_url"
                                                    ...
                                                    class="w-20 h-20 rounded-full ring-4 ring-pink-100 shadow-lg" />
                                                <div class="absolute -bottom-1 -right-1 w-7 h-7 bg-gradient-to-br from-pink-500 to-purple-500 rounded-full flex items-center justify-center shadow-lg">
                                            ...
                                            <div>
                                                <h2 class="text-xl font-bold text-gray-900">@{{ overview.account?.username }}</h2>
                                                <p class="text-gray-500">{{ overview.account?.name }}</p>
                                            </div>
                                        </div>

                                        <!-- Health Score -->
                                        <div class="flex-1 lg:text-center">
                                            <button @click="showHealthModal = true" class="inline-flex items-center gap-3 px-6 py-3 rounded-2xl transition-all hover:scale-105"
                                                :class="{
                                                    'bg-green-100 hover:bg-green-200': healthStatus.color === 'green',
                                                    'bg-blue-100 hover:bg-blue-200': healthStatus.color === 'blue',
                                                    'bg-yellow-100 hover:bg-yellow-200': healthStatus.color === 'yellow',
                                                    'bg-orange-100 hover:bg-orange-200': healthStatus.color === 'orange',
                                                }">
                                                <span class="text-3xl">{{ healthStatus.emoji }}</span>
                                                <div class="text-left">
                                                    <div class="text-sm font-medium text-gray-600">Akkount holati</div>
                                                    <div class="text-xl font-bold" :class="{
                                                        'text-green-700': healthStatus.color === 'green',
                                                        'text-blue-700': healthStatus.color === 'blue',
                                                        'text-yellow-700': healthStatus.color === 'yellow',
                                                        'text-orange-700': healthStatus.color === 'orange',
                                        ...
                                        <div class="grid grid-cols-3 gap-6 lg:gap-8">
                                            <div class="text-center">
                                                <div class="text-2xl lg:text-3xl font-bold text-gray-900">{{ formatNumber(overview.account?.followers_count) }}</div>
                                                <div class="text-sm text-gray-500">Follower</div>
                                            </div>
                                            <div class="text-center">
                                                <div class="text-2xl lg:text-3xl font-bold text-gray-900">{{ formatNumber(overview.account?.media_count) }}</div>
                    ...
                    <div class="rounded-2xl p-5 border-2" :class="getEngagementBg(overview?.account?.engagement_rate)">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                <h3 class="font-bold text-gray-900 mb-1">Engagement Rate nima?</h3>
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        <div class="bg-white rounded-2xl shadow-lg p-5 hover:shadow-xl transition-all">
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-sm font-semibold text-gray-500">Yetib borgan odamlar</span>
                                <span class="text-2xl">👥</span>
                            </div>
                            <div class="text-3xl font-bold text-gray-900">{{ formatNumber(overview?.current?.reach) }}</div>
                            <div class="flex items-center gap-1 mt-2">
                                <span :class="(overview?.change?.reach || 0) >= 0 ? 'text-green-600' : 'text-red-600'" class="text-sm font-semibold">
                                    {{ formatChange(overview?.change?.reach || 0) }}
                                </span>
                                <span class="text-xs text-gray-500">o'tgan davrga nisbatan</span>
                            </div>
                        </div>

                        <div class="bg-white rounded-2xl shadow-lg p-5 hover:shadow-xl transition-all">
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-sm font-semibold text-gray-500">Ko'rishlar soni</span>
                                <span class="text-2xl">👁️</span>
                            </div>
                            <div class="text-3xl font-bold text-gray-900">{{ formatNumber(overview?.current?.impressions) }}</div>
                            <div class="flex items-center gap-1 mt-2">
                                <span :class="(overview?.change?.impressions || 0) >= 0 ? 'text-green-600' : 'text-red-600'" class="text-sm font-semibold">
                        ...
                        <div class="bg-white rounded-2xl shadow-lg p-5 hover:shadow-xl transition-all">
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-sm font-semibold text-gray-500">Likelar</span>
                                <span class="text-2xl">❤️</span>
                            </div>
                            <div class="text-3xl font-bold text-gray-900">{{ formatNumber(overview?.current?.total_likes) }}</div>
                            <div class="flex items-center gap-1 mt-2">
                                <span :class="(overview?.change?.total_likes || 0) >= 0 ? 'text-green-600' : 'text-red-600'" class="text-sm font-semibold">
                        ...
                        <div class="bg-white rounded-2xl shadow-lg p-5 hover:shadow-xl transition-all">
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-sm font-semibold text-gray-500">Izohlar</span>
                                <span class="text-2xl">💬</span>
                            </div>
                            <div class="text-3xl font-bold text-gray-900">{{ formatNumber(overview?.current?.total_comments) }}</div>
                            <div class="flex items-center gap-1 mt-2">
                                <span :class="(overview?.change?.total_comments || 0) >= 0 ? 'text-green-600' : 'text-red-600'" class="text-sm font-semibold">
                            ...
                            <nav class="flex overflow-x-auto px-2">
                                <template v-for="tab in tabs" :key="tab.id">
                                    <a v-if="tab.isLink" :href="tab.href"
                                        class="px-6 py-4 text-sm font-semibold border-b-2 whitespace-nowrap transition-all border-transparent text-gray-500 hover:text-gray-700 hover:bg-white/50 flex items-center gap-2">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="p-6">
                            ...
                            <div v-if="activeTab === 'insights'" class="space-y-8">
                                ...
                                <template v-else>
                                    ...
                                    <div class="bg-gradient-to-r from-purple-600 to-pink-600 rounded-3xl p-8 text-white">
                                        <h2 class="text-2xl font-bold mb-2">Sizning biznes savollaringizga javoblar</h2>
                                        <p class="text-purple-100 mb-6">Instagram ma'lumotlaringizga asoslangan amaliy tavsiyalar</p>

                                        <div class="grid gap-4">
                                            <!-- Strategy Summary -->
                                            <div v-if="businessInsights?.best_posting_strategy?.strategy_summary" class="bg-white/10 backdrop-blur rounded-2xl p-5">
                                                <div class="flex items-start gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="text-purple-100 mt-1">{{ businessInsights.best_posting_strategy.strategy_summary }}</p>
                                                    </div>
                                                </div>
                                            </div>

                                            <!-- Growth Insight -->
                                            <div v-if="growthDrivers?.insight" class="bg-white/10 backdrop-blur rounded-2xl p-5">
                                                <div class="flex items-start gap-4">
                                                    <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center flex-shrink-0">
                                                        <span class="text-2xl">📈</span>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <p class="text-purple-100 mt-1">{{ growthDrivers.insight }}</p>
                                                    </div>
                                                </div>
                                            </div>

                                            <!-- Viral Formula -->
                                            <div v-if="viralAnalysis?.viral_formula" class="bg-white/10 backdrop-blur rounded-2xl p-5">
                                                <div class="flex items-start gap-4">
                                                    <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center flex-shrink-0">
                                                        <span class="text-2xl">🚀</span>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <p class="text-purple-100 mt-1">{{ viralAnalysis.viral_formula }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div v-if="contentWinners">
                                        <div class="flex items-center justify-between mb-6">
                                            <div>
                                                <h3 class="text-xl font-bold text-gray-900">Eng yaxshi kontentlaringiz</h3>
                                                <p class="text-gray-500">Har bir kategoriyada g'olib postlar - bu formatlarni takrorlang!</p>
                                            </div>
                                            <!-- View Toggle -->
                                            <div class="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
                                                <button @click="winnersViewMode = 'cards'"
                                                    :class="winnersViewMode === 'cards' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'"
                                                ...
                                                <button @click="winnersViewMode = 'table'"
                                                    :class="winnersViewMode === 'table' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'"
                                                    class="px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div v-if="winnersViewMode === 'cards'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                            <template v-for="(winner, key) in contentWinners" :key="key">
                                                <div v-if="winner" class="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-xl transition-all group">
                                                    ...
                                                    <div class="p-4">
                                                        ...
                                                        <p v-if="winner.why_it_worked" class="text-sm text-green-600 bg-green-50 rounded-lg p-2">
                                                            {{ winner.why_it_worked }}
                                                        </p>
                                                        <a v-if="winner.permalink" :href="winner.permalink" target="_blank"
                                                            class="mt-3 inline-flex items-center text-sm text-pink-600 hover:text-pink-700 font-medium">
                                            ...
                                            <div class="overflow-x-auto">
                                                <table class="w-full">
                                                    <thead class="bg-gray-50 border-b border-gray-200">
                                                        <tr>
                                                            <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Kontent</th>
                                                            <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Kategoriya</th>
                                                            <th class="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Reach</th>
                                                            <th class="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Engagement</th>
                                                            <th class="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Likes</th>
                                                            <th class="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Comments</th>
                                                            <th class="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Saves</th>
                                                            <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Sana</th>
                                                            <th class="text-center px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Link</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody class="divide-y divide-gray-100">
                                                        <template v-for="(winner, key) in contentWinners" :key="key">
                                                            <tr v-if="winner" class="hover:bg-gray-50 transition-colors">
                                                                <td class="px-6 py-4">
                                                                ...
                                                                <td class="px-6 py-4 text-right">
                                                                    <span class="text-sm font-bold text-gray-900">{{ formatNumber(winner.reach) }}</span>
                                                                </td>
                                                                <td class="px-6 py-4 text-right">
                                                                    <span class="text-sm font-medium" :class="winner.engagement_rate > 3 ? 'text-green-600' : winner.engagement_rate > 1 ? 'text-blue-600' : 'text-gray-600'">
                                                                        {{ winner.engagement_rate }}%
                                                                    </span>
                                                                </td>
                                                                <td class="px-6 py-4 text-right">
                                                                    <span class="text-sm text-gray-600">{{ formatNumber(winner.likes) }}</span>
                                                                </td>
                                                                <td class="px-6 py-4 text-right">
                                                                    <span class="text-sm text-gray-600">{{ formatNumber(winner.comments) }}</span>
                                                                </td>
                                                                <td class="px-6 py-4 text-right">
                                                                    <span class="text-sm text-gray-600">{{ formatNumber(winner.saves) }}</span>
                                                                </td>
                                                                <td class="px-6 py-4">
                                                                    <span class="text-sm text-gray-500">{{ winner.posted_at }}</span>
                                                                </td>
                                                                <td class="px-6 py-4 text-center">
                                                                    <a v-if="winner.permalink" :href="winner.permalink" target="_blank"
                                                                        class="inline-flex items-center justify-center w-8 h-8 bg-pink-100 text-pink-600 rounded-lg hover:bg-pink-200 transition-colors">
                                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                                                                        </svg>
                                                                    </a>
                                    ...
                                    <div v-if="businessInsights?.best_posting_strategy">
                                        <div class="flex items-center justify-between mb-6">
                                            <div>
                                                <h3 class="text-xl font-bold text-gray-900">Eng yaxshi post qilish vaqtlari</h3>
                                                <p class="text-gray-500">Sizning auditoriyangiz qachon eng faol?</p>
                                            </div>
                                            <!-- View Toggle -->
                                            <div class="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
                                                <button @click="timesViewMode = 'cards'"
                                                    :class="timesViewMode === 'cards' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'"
                                                    class="px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                                                    </svg>
                                                ...
                                                <button @click="timesViewMode = 'table'"
                                                    :class="timesViewMode === 'table' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'"
                                                    class="px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M3 14h18m-9-4v8m-7 0h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                                    </svg>
                                            ...
                                            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                                                ...
                                                <div class="space-y-3">
                                                    <div v-for="(day, index) in businessInsights.best_posting_strategy.best_days?.slice(0, 3)" :key="index"
                                                        ...
                                                        :class="index === 0 ? 'bg-green-50 border border-green-200' : 'bg-gray-50'">
                                                        <div class="flex items-center gap-3">
                                                        ...
                                                        <div class="text-right">
                                                            <div class="text-sm font-bold text-gray-900">{{ formatNumber(day.avg_reach) }} reach</div>
                                                            <div class="text-xs text-gray-500">{{ day.posts_count }} post</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                                                ...
                                                <div class="space-y-3">
                                                    <div v-for="(hour, index) in businessInsights.best_posting_strategy.best_hours?.slice(0, 3)" :key="index"
                                                        class="flex items-center justify-between p-3 rounded-xl"
                                                        :class="index === 0 ? 'bg-blue-50 border border-blue-200' : 'bg-gray-50'">
                                                        <div class="flex items-center gap-3">
                                                            <span v-if="index === 0" class="text-xl">🥇</span>
                                                            <span v-else-if="index === 1" class="text-xl">🥈</span>
                                                            <span v-else class="text-xl">🥉</span>
                                                            <span class="font-semibold text-gray-900">{{ hour.hour }}</span>
                                                        </div>
                                                        <div class="text-right">
                                                            <div class="text-sm font-bold text-gray-900">{{ formatNumber(hour.avg_reach) }} reach</div>
                                                            <div class="text-xs text-gray-500">{{ hour.posts_count }} post</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                                                ...
                                                <table class="w-full">
                                                    <thead class="bg-gray-50 border-b border-gray-100">
                                                        <tr>
                                                            <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Kun</th>
                                                            <th class="text-right px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Postlar</th>
                                                            <th class="text-right px-6 py-3 text-xs font-semibold text-gray-500 uppercase">O'rt. Reach</th>
                                                            <th class="text-right px-6 py-3 text-xs font-semibold text-gray-500 uppercase">O'rt. Engagement</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody class="divide-y divide-gray-100">
                                                        <tr v-for="(day, index) in businessInsights.best_posting_strategy.best_days" :key="index"
                                                            ...
                                                            <td class="px-6 py-3 text-right text-sm text-gray-600">{{ day.posts_count }}</td>
                                                            <td class="px-6 py-3 text-right text-sm font-bold text-gray-900">{{ formatNumber(day.avg_reach) }}</td>
                                                            <td class="px-6 py-3 text-right text-sm text-gray-600">{{ day.avg_engagement }}%</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                                ...
                                                <div class="max-h-80 overflow-y-auto">
                                                    <table class="w-full">
                                                        <thead class="bg-gray-50 border-b border-gray-100 sticky top-0">
                                                            <tr>
                                                                <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Soat</th>
                                                                <th class="text-right px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Postlar</th>
                                                                <th class="text-right px-6 py-3 text-xs font-semibold text-gray-500 uppercase">O'rt. Reach</th>
                                                                <th class="text-right px-6 py-3 text-xs font-semibold text-gray-500 uppercase">O'rt. Engagement</th>
                                                            </tr>
                                                            ...
                                                            <tr v-for="(hour, index) in businessInsights.best_posting_strategy.best_hours" :key="index"
                                                                ...
                                                                <td class="px-6 py-3">
                                                                    <div class="flex items-center gap-2">
                                                                        ...
                                                                        <span v-else-if="index === 1" class="text-sm">🥈</span>
                                                                        <span v-else-if="index === 2" class="text-sm">🥉</span>
                                                                        <span class="font-medium text-gray-900">{{ hour.hour }}</span>
                                                                    </div>
                                                                </td>
                                                                <td class="px-6 py-3 text-right text-sm text-gray-600">{{ hour.posts_count }}</td>
                                                                <td class="px-6 py-3 text-right text-sm font-bold text-gray-900">{{ formatNumber(hour.avg_reach) }}</td>
                                                                <td class="px-6 py-3 text-right text-sm text-gray-600">{{ hour.avg_engagement }}%</td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>
                                    ...
                                    <div v-if="viralAnalysis?.viral_posts?.length > 0">
                                        <div class="flex items-center justify-between mb-6">
                                            <div>
                                                <h3 class="text-xl font-bold text-gray-900">Viral bo'lgan postlaringiz</h3>
                                                <p class="text-gray-500">Bu postlar followerlaringiz sonidan ko'proq odamga yetib borgan</p>
                                            </div>
                                            <!-- View Toggle -->
                                            <div class="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
                                                <button @click="viralViewMode = 'cards'"
                                                    :class="viralViewMode === 'cards' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'"
                                                    class="px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                                                    </svg>
                                                ...
                                                <button @click="viralViewMode = 'table'"
                                                    :class="viralViewMode === 'table' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'"
                                                    class="px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M3 14h18m-9-4v8m-7 0h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                                    </svg>
                                        ...
                                        <div v-if="viralViewMode === 'cards'" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                                            <a v-for="post in viralAnalysis.viral_posts.slice(0, 10)" :key="post.id"
                                                ...
                                                class="group relative aspect-square bg-gray-100 rounded-2xl overflow-hidden hover:shadow-xl transition-all">
                                                <img v-if="post.thumbnail_url" :src="post.thumbnail_url" class="w-full h-full object-cover" />
                                                <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent opacity-0 group-hover:opacity-100 transition-all">
                                            ...
                                            <div class="overflow-x-auto">
                                                <table class="w-full">
                                                    <thead class="bg-gray-50 border-b border-gray-200">
                                                        <tr>
                                                            <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Kontent</th>
                                                            <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Turi</th>
                                                            <th class="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Reach</th>
                                                            <th class="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Viral Ratio</th>
                                                            <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Caption</th>
                                                            <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Sana</th>
                                                            <th class="text-center px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Link</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody class="divide-y divide-gray-100">
                                                        <tr v-for="post in viralAnalysis.viral_posts" :key="post.id" class="hover:bg-gray-50 transition-colors">
                                                            <td class="px-6 py-4">
                                                                <div class="flex items-center gap-3">
                                                            ...
                                                            <td class="px-6 py-4 text-right">
                                                                <span class="text-sm font-bold text-gray-900">{{ formatNumber(post.reach) }}</span>
                                                            </td>
                                                            <td class="px-6 py-4 text-right">
                                                            ...
                                                            <td class="px-6 py-4">
                                                                <span class="text-sm text-gray-600 max-w-xs truncate block">{{ post.caption }}</span>
                                                            </td>
                                                            <td class="px-6 py-4">
                                    ...
                                    <div v-if="businessInsights?.content_recommendations" class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl border border-amber-200 p-6">
                                        <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                                        ...
                                        <div class="space-y-4">
                                            <div v-for="(item, index) in businessInsights.content_recommendations.action_items" :key="index"
                                                class="flex items-start gap-3 p-4 bg-white rounded-xl shadow-sm">
                                                <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                                                    :class="item.priority === 'high' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'">
                                                    {{ index + 1 }}
                                                </div>
                                                <div>
                                                    <span v-if="item.priority === 'high'" class="text-xs font-bold text-red-600 uppercase">Muhim</span>
                                                    <p class="text-gray-800 font-medium">{{ item.action }}</p>
                                                </div>
                                            </div>

                                            <!-- Top Hashtags -->
                                            <div v-if="businessInsights.content_recommendations.top_hashtags?.length" class="mt-4 p-4 bg-white rounded-xl">
                                                <h4 class="font-semibold text-gray-900 mb-2">Eng samarali hashtaglar:</h4>
                                    ...
                                    <!-- Performance Trend -->
                                    <div v-if="businessInsights?.performance_trends" class="bg-white rounded-2xl border border-gray-200 p-6">
                                        <h3 class="text-lg font-bold text-gray-900 mb-4">Sizning trend</h3>

                                        <div class="flex items-center gap-4 p-4 rounded-xl"
                                            :class="{
                                                'bg-green-50 border border-green-200': businessInsights.performance_trends.overall_trend === 'growing',
                                                'bg-red-50 border border-red-200': businessInsights.performance_trends.overall_trend === 'declining',
                                                'bg-gray-50 border border-gray-200': businessInsights.performance_trends.overall_trend === 'stable'
                                            }">
                                            <span class="text-4xl">
                                            ...
                                            <div>
                                                <div class="font-bold text-lg"
                                                    :class="{
                                                        'text-green-700': businessInsights.performance_trends.overall_trend === 'growing',
                                                        'text-red-700': businessInsights.performance_trends.overall_trend === 'declining',
                                                        'text-gray-700': businessInsights.performance_trends.overall_trend === 'stable'
                                                    }">
                                                ...
                                                <p class="text-gray-600">{{ businessInsights.performance_trends.trend_insight }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                                            <div v-for="(value, key) in businessInsights.performance_trends.changes" :key="key"
                                                class="text-center p-3 bg-gray-50 rounded-xl">
                                                <div class="text-lg font-bold"
                                                    :class="value > 0 ? 'text-green-600' : value < 0 ? 'text-red-600' : 'text-gray-600'">
                                                    {{ value > 0 ? '+' : '' }}{{ value }}%
                                                </div>
                                    ...
                                    <!-- Audience Profile -->
                                    <div v-if="businessInsights?.audience_insights" class="bg-white rounded-2xl border border-gray-200 p-6">
                                        <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                                            <span class="text-2xl">👥</span> Auditoriyangiz kim?
                                        </h3>

                                        <div class="p-4 bg-purple-50 rounded-xl border border-purple-200 mb-4">
                                            <p class="text-purple-800 font-medium">{{ businessInsights.audience_insights.target_audience_profile }}</p>
                                        </div>
                            ...
                            <div v-if="activeTab === 'overview'" class="space-y-8">
                                ...
                                <div v-if="contentComparison">
                                    <h3 class="text-xl font-bold text-gray-900 mb-2">Qaysi turdagi kontent yaxshi ishlaydi?</h3>
                                    <p class="text-gray-500 mb-6">Har xil turdagi postlaringiz qanday natija berayotganini ko'ring</p>
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                                        <div v-for="(data, type) in contentComparison" :key="type"
                                            class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border border-gray-200 hover:shadow-lg transition-all">
                                            <div class="flex items-center justify-between mb-5">
                                                <div class="flex items-center gap-3">
                                                    <span class="text-2xl">{{ type === 'Reels' ? '🎬' : type === 'Carousel' ? '📷' : '🖼️' }}</span>
                                                    <span class="font-bold text-gray-900 text-lg">{{ type }}</span>
                                                </div>
                                                <span class="text-sm font-medium text-gray-500 bg-white px-3 py-1.5 rounded-full shadow-sm">{{ data.count }} ta</span>
                                            </div>
                                            <div class="space-y-4">
                                                <div>
                                                    <div class="flex justify-between items-center mb-1">
                                                        <span class="text-sm text-gray-500">O'rtacha Reach</span>
                                                        <span class="font-bold text-gray-900">{{ formatNumber(data.avg_reach) }}</span>
                                                    </div>
                                                    <div class="text-xs text-gray-400">Necha odamga yetib borgan</div>
                                                </div>
                                                <div>
                                                    <div class="flex justify-between items-center mb-1">
                                                    ...
                                                    <div class="text-xs text-gray-400">Faollik darajasi</div>
                                                </div>
                                                <div>
                                                    <div class="flex justify-between items-center mb-1">
                                                        <span class="text-sm text-gray-500">Jami Like</span>
                                                        <span class="font-bold text-gray-900">{{ formatNumber(data.total_likes) }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div v-if="mediaPerformance?.top_posts?.length">
                                    <h3 class="text-xl font-bold text-gray-900 mb-2">Eng yaxshi ishlab postlar</h3>
                                    <p class="text-gray-500 mb-6">Bu postlar eng ko'p like va izoh olgan - shu tarzda davom eting!</p>
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div v-for="(post, index) in mediaPerformance.top_posts.slice(0, 4)" :key="post.id"
                                            class="group relative rounded-2xl overflow-hidden aspect-square bg-gray-100 shadow-lg hover:shadow-2xl transition-all hover:-translate-y-1">
                                            <img v-if="post.thumbnail_url || post.media_url" :src="post.thumbnail_url || post.media_url"
                                                class="w-full h-full object-cover" />
                                            <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent">
                                                <div class="absolute top-3 left-3">
                                                    <span class="inline-flex items-center gap-1 px-2 py-1 bg-white/90 backdrop-blur-sm rounded-lg text-xs font-bold text-gray-900">
                                ...
                                <div v-if="engagement?.best_posting_days?.length || engagement?.best_posting_hours?.length"
                                    ...
                                    <h3 class="text-xl font-bold text-gray-900 mb-2">Qachon post qilish kerak?</h3>
                                    <p class="text-gray-500 mb-6">Auditoriyangiz eng faol bo'lgan vaqtlar</p>
                                    <div class="grid md:grid-cols-2 gap-6">
                                        <div v-if="engagement?.best_posting_days?.length">
                                            <h4 class="font-semibold text-gray-700 mb-3">Eng yaxshi kunlar:</h4>
                                            <div class="flex flex-wrap gap-2">
                                                <span v-for="(day, i) in engagement.best_posting_days" :key="day"
                                                    class="px-4 py-2 rounded-xl font-medium"
                                                    :class="i === 0 ? 'bg-purple-500 text-white' : 'bg-white text-gray-700 shadow-sm'">
                                                    {{ i === 0 ? '⭐ ' : '' }}{{ day }}
                                                </span>
                                            </div>
                                        </div>
                                        <div v-if="engagement?.best_posting_hours?.length">
                                            <h4 class="font-semibold text-gray-700 mb-3">Eng faol soatlar:</h4>
                                            <div class="flex flex-wrap gap-2">
                                                <span v-for="(hour, i) in engagement.best_posting_hours" :key="hour"
                                                    class="px-4 py-2 rounded-xl font-medium"
                                                    :class="i === 0 ? 'bg-pink-500 text-white' : 'bg-white text-gray-700 shadow-sm'">
                                                    {{ hour }}:00
                                                </span>
                                            </div>
                                        </div>
                            ...
                            <div v-if="activeTab === 'reels'" class="space-y-6">
                                <div v-if="reelsAnalytics?.summary" class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <div class="bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl p-6 text-white shadow-xl">
                                        ...
                                        <div class="text-purple-100 text-sm mt-2">Jami ko'rishlar</div>
                                        <div class="text-xs text-purple-200 mt-1">Reelslaringizni necha marta ko'rishgan</div>
                                    </div>
                                    <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                                        <div class="text-4xl font-bold text-gray-900">{{ reelsAnalytics.summary.total_reels }}</div>
                                        <div class="text-gray-500 text-sm mt-2">Reelslar soni</div>
                                    </div>
                                    <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                                        <div class="text-4xl font-bold text-gray-900">{{ formatNumber(reelsAnalytics.summary.avg_plays) }}</div>
                                        <div class="text-gray-500 text-sm mt-2">O'rtacha ko'rish</div>
                                        <div class="text-xs text-gray-400 mt-1">Har bir reels uchun</div>
                                    </div>
                                    <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                                        ...
                                        <div class="text-gray-500 text-sm mt-2">Engagement</div>
                                    </div>
                                </div>

                                <!-- Top Reels by Reach -->
                                <div v-if="reelsAnalytics?.most_viewed?.length">
                                    <h3 class="text-xl font-bold text-gray-900 mb-2">Eng ko'p yetib borgan Reelslar</h3>
                                    <p class="text-gray-500 mb-6">Bu reelslar eng ko'p odamga yetib borgan - shu tarzda davom eting!</p>
                                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                        <div v-for="(reel, index) in reelsAnalytics.most_viewed" :key="reel.id"
                                            class="group relative rounded-2xl overflow-hidden aspect-[9/16] bg-gray-100 shadow-lg hover:shadow-2xl transition-all hover:-translate-y-1">
                                            ...
                                            <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent">
                                                <div class="absolute top-3 right-3">
                                                    <span class="inline-flex items-center gap-1 px-2 py-1 bg-purple-500 rounded-lg text-xs font-bold text-white">
                                                        #{{ index + 1 }}
                                                    </span>
                                                </div>
                                                <div class="absolute bottom-0 left-0 right-0 p-4 text-white">
                                ...
                                <div v-if="reelsAnalytics?.all_reels?.length" class="mt-8">
                                    <h3 class="text-xl font-bold text-gray-900 mb-2">Barcha Reelslar</h3>
                                    <p class="text-gray-500 mb-6">Jami {{ reelsAnalytics.summary?.total_reels || 0 }} ta reels</p>
                                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                                        <a v-for="reel in reelsAnalytics.all_reels.slice(0, 20)" :key="reel.id"
                                            :href="reel.permalink" target="_blank"
                                            class="group relative rounded-2xl overflow-hidden aspect-[9/16] bg-gray-100 shadow-md hover:shadow-xl transition-all hover:-translate-y-1">
                                            <img v-if="reel.thumbnail_url" :src="reel.thumbnail_url" class="w-full h-full object-cover" />
                                            <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent opacity-0 group-hover:opacity-100 transition-all">
                                                <div class="absolute bottom-0 left-0 right-0 p-3 text-white">
                                ...
                                <div v-else-if="!reelsAnalytics?.summary?.total_reels" class="text-center py-12 bg-gray-50 rounded-2xl">
                                    <div class="text-5xl mb-4">🎬</div>
                                    <h3 class="text-lg font-semibold text-gray-900 mb-2">Hali Reels yo'q</h3>
                                    <p class="text-gray-500">Reels joylaganingizda bu yerda statistika ko'rinadi</p>
                                </div>
                            </div>
                            ...
                            <div v-if="activeTab === 'posts'" class="space-y-6">
                                <!-- Filters -->
                                <div class="flex flex-wrap items-center gap-4 pb-5 border-b border-gray-100">
                                    <div class="flex items-center gap-2">
                                        <label class="text-sm font-medium text-gray-600">Turi:</label>
                                        <select v-model="mediaTypeFilter" class="text-sm border-gray-200 rounded-xl bg-gray-50 focus:bg-white">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <label class="text-sm font-medium text-gray-600">Tartiblash:</label>
                                        <select v-model="mediaSortBy" class="text-sm border-gray-200 rounded-xl bg-gray-50 focus:bg-white">
                                    ...
                                    <div class="flex-1 min-w-[200px]">
                                        <input v-model="mediaSearch" type="text" placeholder="Post qidirish..."
                                            class="w-full text-sm border-gray-200 rounded-xl bg-gray-50 focus:bg-white px-4 py-2" />
                                    </div>
                                    <span class="text-sm font-medium text-gray-500 bg-gray-100 px-3 py-1.5 rounded-full">{{ filteredMedia.length }} ta post</span>
                                </div>

                                <!-- Media Grid -->
                                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    <div v-for="post in paginatedMedia" :key="post.id"
                                        class="group relative rounded-2xl overflow-hidden aspect-square bg-gray-100 shadow-md hover:shadow-xl transition-all hover:-translate-y-1">
                                        <img v-if="post.thumbnail_url || post.media_url" :src="post.thumbnail_url || post.media_url"
                                            class="w-full h-full object-cover" />
                                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent opacity-0 group-hover:opacity-100 transition-all">
                                            <div class="absolute bottom-0 left-0 right-0 p-4 text-white">
                                        ...
                                        <div class="absolute top-2 left-2 text-lg bg-white/90 backdrop-blur-sm rounded-lg px-1.5 py-0.5">
                                ...
                                <div v-if="totalMediaPages > 1" class="flex items-center justify-center gap-3 pt-6">
                                    <button @click="currentPage--" :disabled="currentPage === 1"
                                        class="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-xl text-sm font-medium disabled:opacity-50 hover:bg-gray-200 transition-all">
                                        ← Oldingi
                                    </button>
                                    <span class="px-4 py-2 bg-pink-100 text-pink-700 rounded-xl text-sm font-bold">
                            ...
                            <div v-if="activeTab === 'audience'" class="space-y-6">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <!-- Gender -->
                                    <div v-if="audience?.gender_distribution" class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border border-gray-200">
                                        <h4 class="font-bold text-gray-900 mb-2">Followerlar jinsi</h4>
                                        <p class="text-sm text-gray-500 mb-5">Sizni kim kuzatadi?</p>
                                        <div class="space-y-4">
                                            <div>
                                                <div class="flex items-center justify-between mb-2">
                                                    ...
                                                    <span class="text-2xl font-bold text-gray-900">{{ audience.gender_distribution.male }}%</span>
                                                </div>
                                            ...
                                            <div>
                                                <div class="flex items-center justify-between mb-2">
                                                    <span class="flex items-center gap-2 text-pink-600 font-semibold">
                                                        <span class="text-xl">👩</span> Ayollar
                                                    </span>
                                                    <span class="text-2xl font-bold text-gray-900">{{ audience.gender_distribution.female }}%</span>
                                                </div>
                                                <div class="h-4 bg-gray-200 rounded-full overflow-hidden">
                                                    <div class="h-full bg-gradient-to-r from-pink-400 to-pink-600 rounded-full transition-all"
                                    ...
                                    <!-- Age -->
                                    <div v-if="audience?.age_distribution?.length" class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border border-gray-200">
                                        <h4 class="font-bold text-gray-900 mb-2">Followerlar yoshi</h4>
                                        <p class="text-sm text-gray-500 mb-5">Eng ko'p qaysi yoshdagilar kuzatadi?</p>
                                        <div class="space-y-3">
                                            <div v-for="(age, index) in audience.age_distribution" :key="age.range"
                                                class="flex items-center gap-3">
                                                <span class="text-sm font-medium text-gray-600 w-14">{{ age.range }}</span>
                                                ...
                                                <span class="text-sm font-bold text-gray-900 w-12 text-right">{{ age.percentage }}%</span>
                                    ...
                                    <!-- Top Cities -->
                                    <div v-if="audience?.top_cities?.length" class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border border-gray-200">
                                        <h4 class="font-bold text-gray-900 mb-2">Top shaharlar</h4>
                                        <p class="text-sm text-gray-500 mb-5">Followerlaringiz qayerda yashaydi?</p>
                                        <div class="space-y-3">
                                            <div v-for="(city, i) in audience.top_cities.slice(0, 5)" :key="city.name"
                                                ...
                                                <div class="flex items-center gap-3">
                                                    <span class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm"
                                                        :class="i === 0 ? 'bg-yellow-400 text-yellow-900' : i === 1 ? 'bg-gray-300 text-gray-700' : i === 2 ? 'bg-orange-300 text-orange-800' : 'bg-gray-100 text-gray-600'">
                                                        {{ i + 1 }}
                                                    </span>
                                                    <span class="font-medium text-gray-700">{{ city.name }}</span>
                                                </div>
                                                <span class="font-bold text-gray-900">{{ formatNumber(city.count) }}</span>
                                            </div>
                                        </div>
                                    </div>

                                    <!-- Top Countries -->
                                    <div v-if="audience?.top_countries?.length" class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border border-gray-200">
                                        <h4 class="font-bold text-gray-900 mb-2">Top davlatlar</h4>
                                        <p class="text-sm text-gray-500 mb-5">Qaysi davlatlardan kuzatishadi?</p>
                                        <div class="space-y-3">
                                            <div v-for="(country, i) in audience.top_countries.slice(0, 5)" :key="country.name"
                                                ...
                                                <span class="font-bold text-gray-900">{{ formatNumber(country.count) }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
