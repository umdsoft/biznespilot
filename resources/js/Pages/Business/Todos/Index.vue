...
const openDetailModal = async (todo) => {
    ...
    try {
        const response = await fetch(route('business.todos.show', todo.id), {
            headers: {
        ...
        if (response.ok) {
            ...
            selectedTodo.value = data.todo; // API returns { success: true, todo: {...} }
...
const handleDetailUpdate = async (todoId, formData) => {
    try {
        const response = await fetch(route('business.todos.update', todoId), {
            method: 'PUT',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Accept': 'application/json',
...
const handleDetailToggleSubtask = async (todo, subtask) => {
    try {
        ...
        if (response.ok) {
            // Reload todo data
            const todoResponse = await fetch(route('business.todos.show', todo.id), {
...
const handleDetailAddSubtask = async (todo, title) => {
    try {
        const response = await fetch(route('business.todos.subtasks.store', todo.id), {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Accept': 'application/json',
