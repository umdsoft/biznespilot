<template>
    <BusinessLayout title="Kontentni Tahrirlash">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            <label
                                ...
                                :class="form.platforms.includes(platform.value)
...
// Helpers
const goBack = () => {
    router.get(`/business/marketing/content/${props.post.id}`)
}
...
const submitForm = () => {
    ...
    if (hashtagInput.value) {
        form.value.hashtags = hashtagInput.value.split(',').map(t => t.trim()).filter(t => t)
    }

    // Combine date and time into scheduled_at
    if (form.value.status === 'scheduled' && form.value.scheduled_date && form.value.scheduled_time) {
        form.value.scheduled_at = `${form.value.scheduled_date} ${form.value.scheduled_time}`
    } else {
    ...
    const formData = {
        ...form.value,
        platform: form.value.platforms
    }

    router.put(`/business/marketing/content/${props.post.id}`, formData, {
        preserveScroll: true,
        onSuccess: () => {
