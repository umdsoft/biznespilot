<template>
  <BusinessLayout title="Marketing Kanallari">
...
const submit = () => {
  if (isEditing.value) {
    form.put(`/marketing/channels/${editingChannel.value.id}`, {
      onSuccess: () => {
        closeModal();
      },
    });
  } else {
    form.post('/marketing/channels', {
      onSuccess: () => {
        closeModal();
      },
    });
...
const confirmDelete = (channel) => {
  if (confirm(`Rostdan ham "${channel.name}" kanalini o'chirmoqchimisiz?`)) {
    router.delete(`/marketing/channels/${channel.id}`);
  }
};
