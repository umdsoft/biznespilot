  ...
  <BusinessLayout :title="bot.first_name">
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Link
          :href="route('business.telegram-funnels.users.index', bot.id)"
        ...
        <Link
          :href="route('business.telegram-funnels.conversations.index', bot.id)"
        ...
        <Link
          :href="route('business.telegram-funnels.triggers.index', bot.id)"
          ...
          <div class="p-6 space-y-4">
            ...
            <div>
              ...
              <p v-if="funnels.length === 0" class="mt-2 text-xs text-yellow-600 dark:text-yellow-400">
                ⚠️ Faol funnel yo'q. Avval funnel yarating va faollashtiring.
              </p>
            </div>
...
const setupWebhook = async () => {
  ...
  try {
    const response = await fetch(route('business.telegram-funnels.setup-webhook', props.bot.id), {
      method: 'POST',
      headers: {
...
const saveSettings = async () => {
  ...
  try {
    const response = await fetch(route('business.telegram-funnels.update', props.bot.id), {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
...
const toggleActive = async () => {
  if (confirm(props.bot.is_active ? 'Botni o\'chirishni xohlaysizmi?' : 'Botni yoqishni xohlaysizmi?')) {
    await fetch(route('business.telegram-funnels.toggle-active', props.bot.id), {
      method: 'POST',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
      }
...
const deleteBot = async () => {
  if (confirm('Botni o\'chirishni xohlaysizmi? Bu amalni ortga qaytarib bo\'lmaydi!')) {
    await fetch(route('business.telegram-funnels.destroy', props.bot.id), {
      method: 'DELETE',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
      }
    })
    router.visit(route('business.telegram-funnels.index'))
  }
}
</script>
