<template>
  <BusinessLayout :title="`Foydalanuvchilar - @${bot.username}`">
    <div class="space-y-6">
      <!-- Header -->
...
const applyFilters = () => {
  router.get(route('business.telegram-funnels.users.index', props.bot.id), {
...
const addTag = async () => {
  ...
  await fetch(route('business.telegram-funnels.users.add-tag', [props.bot.id, selectedUser.value.id]), {
    method: 'POST',
    headers: {
...
const removeTag = async (user, tag) => {
  await fetch(route('business.telegram-funnels.users.remove-tag', [props.bot.id, user.id]), {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
const exportUsers = async () => {
  const response = await fetch(route('business.telegram-funnels.users.export', props.bot.id) + '?' + new URLSearchParams({
