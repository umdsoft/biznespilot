<template>
  <BusinessLayout :title="`Suhbat - ${user.full_name}`">
...
const sendMessage = async () => {
  ...
  try {
    const response = await fetch(route('business.telegram-funnels.conversations.send', [props.bot.id, props.conversation.id]), {
      method: 'POST',
      headers: {
...
const assignToMe = async () => {
  await fetch(route('business.telegram-funnels.conversations.assign', [props.bot.id, props.conversation.id]), {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    }
...
const closeConversation = async () => {
  if (confirm('Suhbatni yopishni xohlaysizmi?')) {
    await fetch(route('business.telegram-funnels.conversations.close', [props.bot.id, props.conversation.id]), {
      method: 'POST',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
      }
...
const reopenConversation = async () => {
  await fetch(route('business.telegram-funnels.conversations.reopen', [props.bot.id, props.conversation.id]), {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    }
