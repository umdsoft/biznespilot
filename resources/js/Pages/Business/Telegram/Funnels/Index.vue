<template>
  <BusinessLayout :title="`${bot.first_name} - Funnellar`">
    <div class="space-y-6">
      <!-- Header -->
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="px-6 pb-6 flex items-center gap-3">
            <Link
              :href="route('business.telegram-funnels.funnels.show', [bot.id, funnel.id])"
...
const createFunnel = async () => {
  ...
  try {
    const response = await fetch(route('business.telegram-funnels.funnels.store', props.bot.id), {
      method: 'POST',
      headers: {
    ...
    if (data.success) {
      router.visit(route('business.telegram-funnels.funnels.show', [props.bot.id, data.funnel.id]))
    }
  } finally {
...
const toggleFunnelActive = async (funnel) => {
  await fetch(route('business.telegram-funnels.funnels.toggle-active', [props.bot.id, funnel.id]), {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    }
...
const duplicateFunnel = async (funnel) => {
  await fetch(route('business.telegram-funnels.funnels.duplicate', [props.bot.id, funnel.id]), {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    }
...
const deleteFunnel = async (funnel) => {
  if (confirm(`"${funnel.name}" funnelini o'chirishni xohlaysizmi?`)) {
    await fetch(route('business.telegram-funnels.funnels.destroy', [props.bot.id, funnel.id]), {
      method: 'DELETE',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
      }
