<template>
  <BusinessLayout :title="`Broadcast - @${bot.username}`">
    <div class="space-y-6">
      <!-- Header -->
      <div class="relative overflow-hidden bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800 dark:from-purple-800 dark:via-purple-900 dark:to-indigo-950 rounded-2xl p-6 md:p-8">
        ...
        <div class="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div class="flex items-center">
            <Link
              :href="route('business.telegram-funnels.show', bot.id)"
          ...
          <Link
            :href="route('business.telegram-funnels.broadcasts.create', bot.id)"
...
const startBroadcast = async (broadcast) => {
  if (confirm('Broadcastni boshlashni xohlaysizmi?')) {
    await fetch(route('business.telegram-funnels.broadcasts.start', [props.bot.id, broadcast.id]), {
      method: 'POST',
      headers: {
...
const pauseBroadcast = async (broadcast) => {
  await fetch(route('business.telegram-funnels.broadcasts.pause', [props.bot.id, broadcast.id]), {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    }
...
const resumeBroadcast = async (broadcast) => {
  await fetch(route('business.telegram-funnels.broadcasts.resume', [props.bot.id, broadcast.id]), {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    }
...
const cancelBroadcast = async (broadcast) => {
  if (confirm('Broadcastni bekor qilishni xohlaysizmi?')) {
    await fetch(route('business.telegram-funnels.broadcasts.cancel', [props.bot.id, broadcast.id]), {
      method: 'POST',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
      }
...
const deleteBroadcast = async (broadcast) => {
  if (confirm('Broadcastni o\'chirishni xohlaysizmi?')) {
    await fetch(route('business.telegram-funnels.broadcasts.destroy', [props.bot.id, broadcast.id]), {
      method: 'DELETE',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
      }
