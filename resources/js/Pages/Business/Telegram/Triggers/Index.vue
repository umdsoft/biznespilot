<template>
  <BusinessLayout :title="`${bot.first_name} - Triggerlar`">
    <div class="space-y-6">
      <!-- Header -->
...
const saveTrigger = async () => {
  ...
  try {
    const url = editingTrigger.value
      ? route('business.telegram-funnels.triggers.update', [props.bot.id, editingTrigger.value.id])
      : route('business.telegram-funnels.triggers.store', props.bot.id)
...
const toggleTriggerActive = async (trigger) => {
  try {
    await fetch(route('business.telegram-funnels.triggers.toggle-active', [props.bot.id, trigger.id]), {
      method: 'POST',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
      }
...
const deleteTrigger = async (trigger) => {
  ...
  try {
    await fetch(route('business.telegram-funnels.triggers.destroy', [props.bot.id, trigger.id]), {
      method: 'DELETE',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
      }
...
const testTrigger = async () => {
  ...
  try {
    const response = await fetch(route('business.telegram-funnels.triggers.test', props.bot.id), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
