<template>
  ...
  >
    ...
    >
      <!-- Completed -->
      <svg v-if="step.is_completed" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
      </svg>
      <!-- Icon based on step code -->
      <component v-else :is="getStepIcon(step.code)" class="w-5 h-5" />
    </div>

    <!-- Content -->
    <div class="flex-1 min-w-0">
      <div class="flex items-center gap-2">
        <h4 class="font-semibold text-gray-900 truncate">{{ step.name_uz || step.name?.uz || step.name }}</h4>
        <span class="px-2 py-0.5 text-xs font-medium bg-blue-50 text-blue-600 rounded">Ixtiyoriy</span>
      </div>
      <p class="text-sm text-gray-500 truncate">{{ step.description_uz || step.description?.uz || step.description }}</p>

      <!-- Progress bar -->
      <div v-if="!step.is_completed && step.completion_percent > 0" class="mt-2">
    ...
    <div class="flex items-center gap-2 flex-shrink-0">
      <!-- Completion percent or time -->
      <span v-if="step.is_completed" class="text-sm font-medium text-green-600">Bajarildi</span>
      <span v-else class="text-xs text-gray-400">~{{ step.estimated_time_minutes || 5 }} daq</span>

      <!-- Arrow -->
      <svg
        class="w-5 h-5"
        :class="step.is_completed ? 'text-green-400' : 'text-gray-400'"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
