...
const toggleStatus = (survey) => {
    router.post(getRoute('custdev.toggle-status', { custdev: survey.id }), {}, {
        preserveScroll: true,
    });
};
...
const inactiveSurveysCount = computed(() => props.surveys?.filter(s => s.status !== 'active').length || 0);
</script>
...
<template>
                        ...
                        >
                            ...
                            <td class="px-4 py-4 text-center">
                                <span class="inline-flex items-center justify-center min-w-[2.5rem] px-3 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-lg text-sm font-semibold">
                                    {{ survey.questions_count || survey.questions?.length || 0 }}
                                </span>
                            </td>
                ...
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-6 text-sm">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-gray-600 dark:text-gray-400">Faol: <span class="font-semibold text-gray-900 dark:text-gray-100">{{ activeSurveysCount }}</span></span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="w-2 h-2 bg-gray-400 rounded-full"></span>
                            <span class="text-gray-600 dark:text-gray-400">Nofaol: <span class="font-semibold text-gray-900 dark:text-gray-100">{{ inactiveSurveysCount }}</span></span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <Teleport to="body">
        <div v-if="deletingSurvey" class="fixed inset-0 z-50 overflow-y-auto">
