                        ...
                        <div class="flex items-center justify-between mt-2">
                            <div>
                                <span class="text-sm font-bold" style="color: var(--tg-theme-text-color)">
                                    {{ service.pricing_type === 'quote' ? "Kelishilgan" : formatPrice(service.base_price || service.price) }}
                                </span>
                                <span v-if="service.pricing_unit" class="text-xs" style="color: var(--tg-theme-hint-color)"> / {{ service.pricing_unit }}</span>
                            </div>
                            <span v-if="service.estimated_minutes" class="text-xs" style="color: var(--tg-theme-hint-color)">~{{ service.estimated_minutes }} min</span>
                        </div>
                    </div>
                </button>
            </div>
...
function goToService(service) {
    hapticImpact('light')
    router.push({ name: 'service-info', params: { slug: service.slug } })
}
