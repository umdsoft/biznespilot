{{-- Pricing Comparison Table - Professional Light Theme --}}
@php
        ...
        <thead>
            <tr>
                <th class="text-left py-5 px-6 bg-gray-50/80 rounded-tl-xl border-b-2 border-gray-200">
                    <span class="text-gray-500 font-medium text-sm">{{ $isUz ? 'Imkoniyatlar' : 'Возможности' }}</span>
                </th>
                @foreach($planHeaders as $index => $header)
                    @php
                        $isBestValue = $header['best_value'] ?? false;
                        $isFree = $header['free'] ?? false;
                    @endphp
                    <th class="py-5 px-4 text-center relative {{ $isBestValue ? '' : 'bg-gray-50/50 border-b-2 border-gray-200' }} {{ $index === 4 ? 'rounded-tr-xl' : '' }}">
                        @if($isBestValue)
                            {{-- Premium card wrapper for best value column --}}
                        ...
                        @else
                            <div class="flex flex-col items-center gap-2">
                                <span class="text-lg font-bold bg-gradient-to-r {{ $header['gradient'] }} bg-clip-text text-transparent">
                                    {{ $header['name'] }}
                                </span>
                            </div>
                        @endif
            ...
            @foreach($categories as $catIndex => $category)
                ...
                <tr class="bg-gradient-to-r from-gray-50 to-white">
                    <td colspan="6" class="py-4 px-6">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-lg bg-{{ $category['color'] }}-100 text-{{ $category['color'] }}-600 flex items-center justify-center">
                                {!! $category['icon'] !!}
                            </div>
                ...
                @foreach($category['features'] as $feature)
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="py-4 px-6 text-gray-600">{{ $feature['name'] }}</td>
                        @foreach($feature['values'] as $vIndex => $value)
                            @php $isBestValueCol = $planHeaders[$vIndex]['best_value'] ?? false; @endphp
                            <td class="py-4 px-4 text-center relative {{ $isBestValueCol ? 'bg-gradient-to-b from-emerald-50 to-teal-50/50' : '' }}">
                                @if($isBestValueCol)
                                ...
                                @elseif($feature['type'] === 'price')
                                    <div class="flex flex-col items-center">
                                        @if($value === 'Bepul' || $value === 'Бесплатно' || $value === '0')
                                            <span class="text-xl font-bold text-green-600">{{ $value === '0' ? ($isUz ? 'Bepul' : 'Бесплатно') : $value }}</span>
                                        @else
                                            <span class="text-xl font-bold text-gray-900">{{ $value }}</span>
                                            <span class="text-xs text-gray-500">{{ $isUz ? 'so\'m' : 'сум' }}</span>
                                        @endif
                                    </div>
                                @else
                                    @if(str_contains($value, 'Cheksiz') || str_contains($value, 'Безлимит'))
        ...
        <tfoot>
            <tr class="border-t-2 border-gray-200 bg-gray-50/80">
                ...
                @foreach($plans as $index => $plan)
                    @php $isBestValueCol = $planHeaders[$index]['best_value'] ?? false; @endphp
                    <td class="py-6 px-4 text-center relative {{ $isBestValueCol ? 'bg-gradient-to-b from-emerald-50 to-teal-100 rounded-b-2xl' : '' }}">
                        @if($isBestValueCol)
