...
<section class="py-24 lg:py-32 relative overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            @foreach($translations['testimonials']['items'] as $index => $testimonial)
                @php $style = $avatarStyles[$index % count($avatarStyles)]; @endphp
                <div class="animate-on-scroll group" style="animation-delay: {{ $index * 0.15 }}s;">
                    <div class="relative h-full bg-white rounded-3xl p-8 shadow-lg shadow-gray-200/50 border border-gray-100 hover:shadow-xl hover:shadow-gray-200/60 transition-all duration-500 hover:-translate-y-2">
                        ...
                        <div class="absolute -top-3 right-6">
                            <div class="{{ $style['statColor'] }} text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg">
                                {{ $testimonial['stat'] ?? '' }}
                            </div>
                        </div>
                        ...
                        <div class="flex space-x-1 mb-6 mt-4">
                            @for($i = 0; $i < 5; $i++)
                                <svg class="w-5 h-5 text-yellow-400 group-hover:scale-110 transition-transform" style="transition-delay: {{ $i * 50 }}ms;" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <!-- SVG Avatar -->
                                <div class="relative">
                                    <div class="w-14 h-14 bg-gradient-to-br {{ $style['gradient'] }} rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
                                        {!! $style['icon'] !!}
                                    </div>
                                    <!-- Online indicator -->
                            ...
                            <div class="hidden sm:block text-right">
                                <div class="text-xs text-gray-400">{{ $testimonial['company_size'] ?? '' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-8">
                    ...
                    @foreach($stats as $index => $stat)
                        <div class="text-center" style="animation-delay: {{ $index * 0.1 }}s;">
                            ...
                            <div class="text-3xl sm:text-4xl font-black text-white mb-2">{{ $stat['value'] }}</div>
